...
import { GET_CLASS, GET_TAG } from "../ShopDetailQueries";
...
export default ({ data }) => {
  ...
  const categories = ShopInfoState.CategoryData;
  ...
  const onChange = (e) => {
    ...
    if (name === "order") {
      if (Number(value) > 0) {
        const rtnData = ShopInfoState.TagInformation.value.map((eachData) => {
  ...
  const deleteRow = (e, rowId) => {
    e.preventDefault();
    let PrevData = ShopInfoState.TagInformation.value;
    ...
    return (
      <tr>
        <td className="orderInputCell">
          <OrderInputBox name="order" defaultValue={data.order} />
        </td>
        <td>
        ...
        <td>
          <select name="tagInfo" defaultValue={data.tagId}>
            <option value={0}>{"-- LOADING --"}</option>
          </select>
        </td>
