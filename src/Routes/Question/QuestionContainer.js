...
export default () => {
	...
	const [updateAdminInfo, { loading: mutationLoading, error: mutationError }] = useMutation(
		QUESTION_MUTATION
	);
	...
	const deleteRow = (e, order) => {
		e.preventDefault();
		setQuestionState(questionState.filter((eachRow) => eachRow.order !== order));
	};
	...
	const onOrderChange = (questionType, e) => {
		...
		setQuestionState(
			questionState.map((option) =>
				option.questionType === questionType ? { ...option, order: Number(value) } : option
			)
		);
	};
