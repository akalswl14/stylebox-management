...
export default ({ data }) => {
  ...
  const ChangeImage = (e) => {
    ...
    reader.onloadend = () => {
      const rtnData = EventInfoState.DetailImages.value.map((eachData) => {
      ...
      if (data.isNewImage) {
        EventInfoDispatch({
          ...
          data: { DetailImages: { value: rtnData, isChange: true } },
        });
      } else {
        EventInfoDispatch({
          type: "UPDATE_DETAILIMAGE",
          data: {
            ...EventInfoState,
            DetailImages: { value: rtnData, isChange: true },
  ...
  const deleteRow = (e, rowId) => {
    e.preventDefault();
    let PrevData = EventInfoState.DetailImages.value;
    ...
    if (isNewImage) {
      EventInfoDispatch({
        type: "UPDATE_DETAILIMAGE",
        data: { DetailImages: { value: PrevData, isChange: true } },
      });
    } else {
      EventInfoDispatch({
        type: "UPDATE_BATCH",
        data: {
          ...EventInfoState,
          DetailImages: { value: PrevData, isChange: true },
          DeleteImageList: [...EventInfoState.DeleteImageList, data.s3Key],
        },
      });
    }
